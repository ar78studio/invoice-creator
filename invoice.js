...
function addCustomItem(renderItems) {
	...
	for (let i = 0; i < customInvoiceItm.length; i++) {
        ...
        <button id="remove-last-entry">delete</button>
...
clientSumName.innerText.length == 0 ? (clientName.disabled = false) : (clientName.disabled = true);
clientSumEmail.innerText.length == 0 ? (clientEmail.disabled = false) : (clientEmail.disabled = true);
